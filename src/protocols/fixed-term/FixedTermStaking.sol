    ...
    FixedTermToken
{
    ...
    /// @notice Error reverted when vault asset is not the same as exchanger input asset
    /// @param vaultAsset the asset of the vault
    /// @param exchangerInAsset the input asset of the exchanger
    error VaultAssetNotSameAsExchangeInAsset(address vaultAsset, address exchangerInAsset);
    ...
    /// @notice Error reverted when redeem from target vault failed
    /// @param sharesAmount the amount of shares being redeemed
    /// @param redeemAmount the amount of underlying tokens being redeemed
    /// @param targetVault the address of the target vault
    error RedeemFailed(uint128 sharesAmount, uint128 redeemAmount, address targetVault);
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < assetsInfoBasket_.length; i++) {
            ...
            if (IERC4626(assetsInfoBasket_[i].targetVault).asset() != UnderlyingTokenExchanger(addrs_[3])._token1()) {
                revert VaultAssetNotSameAsExchangeInAsset(
    ...
    /// @return The total amount of underlying tokens repaid (principal + interest - fees)
        ...
        returns (uint128 repayAmount_)
    {
        ...
        for (uint256 i = 0; i < assetNumberInBasket; i++) {
            AssetInfo memory assetInfo = _assetsInfoBasket[i];
            uint128 assetAmount = (
                (int128(stakeInfo.principal) + interest < 0 ? 0 : uint128(int128(stakeInfo.principal) + interest))
                    * assetInfo.weight + PRECISION - 1
            ) / PRECISION;
            if (assetAmount > 0) {
                uint256 shares = IERC4626(assetInfo.targetVault).convertToShares(assetAmount);
                if (shares > 0) {
                    if (IERC4626(assetInfo.targetVault).redeem(shares, address(this), address(this)) == 0) {
                        revert RedeemFailed(uint128(shares), 0, assetInfo.targetVault);
                    }
                }
            }
        }
    ...
    function feed(uint64 timestamp_) external returns (bool dividends) {
        dividends = _feed(timestamp_, false);
    }

    function feedForce(uint64 timestamp_) external onlyRole(Roles.OPERATOR_ROLE) returns (bool dividends) {
        dividends = _feed(timestamp_, true);
    }

    function _feed(uint64 timestamp_, bool force_) internal returns (bool dividends) {
        ...
        if (interestBearingPrincipal == 0) {
            ...
            int128 interest = int128(getTotalAssetValueInBasket()) - int128(_totalPrincipal) - int128(_totalInterest);
    ...
    /// @notice Method to update the dust balance threshold
    ...
    function updateMaxSupply(uint128 newMaxSupply_) external onlyRole(Roles.OPERATOR_ROLE) {
        ...
        _remainingBalance = newMaxSupply_ - _totalPrincipal;
